...
class Pep
	...
	def initialize(seq, mods=[], mzs, intensities)
	...
	def all_yionstable()
		...
		puts "#{rep}, #{query_no}, #{seq}"
		puts(%w{ mass intensity y-index }.join(","))
  ...
  def assign(ions,tol=1.0)
    ...
    while i1 < mzs.length && i2 < ions.length
      ...
      if dff > tol
        ...
        # puts "ION:\ti1 = #{i1} , i2 = #{i2} => with mass diff: #{mzs[i1]} - #{ions[i2][1]} (mass too large)"
        pkmap[i1] = x
        i2 += 1
        # puts "GOTO\ti2 = #{i2}"
        next
        ...
        pkmap[i1] = x
        # puts "ION:\ti1 = #{i1} , i2 = #{i2} => with mass diff: #{mzs[i1]} - #{ions[i2][1]} (mass too small)"
        i1 += 1
        # i2 = 0
        # puts "GOTO\ti1 = #{i1}"
        next
      ...
      x[0] = i2  + 1
      x[1] = ions[i2][1]
      # puts "ION:\ti1 = #{i1} , i2 = #{i2} => with mass diff: #{mzs[i1]} - #{ions[i2][1]}"
#       if pkmap[i1 - 1][0] == x[0]
#         puts 'here'
